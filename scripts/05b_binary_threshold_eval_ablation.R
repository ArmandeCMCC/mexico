...
#   BOOTSTRAP_N: integer
#
# Ablation hooks (for orchestrated runs via 07b_probcal_ablations.R):
#   ABLATION_NAME: label for this run (sanitized for filenames)
#   REFERENCE_POLICY_TYPE: "precision_floor" | "alerts_per_day_cap" | "f1_max" (default: alerts_per_day_cap)
#   REFERENCE_POLICY_TARGET: numeric (default: 10 for alerts_per_day_cap, 0.10 for precision_floor)
#   FEATURE_INCLUDE_REGEX: keep only features matching this regex
#   FEATURE_EXCLUDE_REGEX: remove features matching this regex
#   OUTPUT_ROOT_OVERRIDE: root for outputs (default: data/baselines/binary_threshold/)
#   FEATURES_PATH_OVERRIDE: path to features file (default: model_ready/features_engineered.rds)
#
# Design: standalone 05b uses legacy default (alerts_per_day_cap=10);
#         07b passes precision_floor=0.10 for headline ablation comparisons.
#
# Examples:
...
#
# Smoke-test with ablation hooks (fast, low bootstrap):
#   ABLATION_NAME=smoke_test \
#   REFERENCE_POLICY_TYPE=precision_floor \
#   REFERENCE_POLICY_TARGET=0.10 \
#   BOOTSTRAP_N=10 \
#   Rscript scripts/05b_binary_threshold_eval.R forecast standard
#
# Verify outputs:
#   - run_config.json: ablation.reference_policy_type == "precision_floor"
#   - operating_thresholds_table.csv: includes row with policy_type="precision_floor", policy_target=0.10
#   - benchmark_table_same_run.csv: operating_policy_type == "precision_floor"
...
# NOTE: RUN_ID is computed later, after ablation hooks are parsed

# Calibration settings
N_CALIBRATION_BINS <- 10
PLATT_SCALING <- TRUE  # Whether to compute Platt-scaled calibration

# EFAULT CALIBRATED METHOD SPEC
...
# ABLATION HOOKS (for orchestrated ablation runs) 
# These allow external scripts to configure ablation-specific settings via environment variables.
# All have sensible defaults so standalone runs continue to work unchanged.
#
# Design note: Legacy defaults are preserved for backward compatibility.
# - Standalone 05b runs: alerts_per_day_cap = 10 (legacy behavior)
# - Orchestrated ablations: pass REFERENCE_POLICY_TYPE/TARGET explicitly for headline policy

# Ablation name/label (for tracking in run_config.json and RUN_ID)
# Sanitize: replace invalid chars with underscore, keep raw version for run_config
ABLATION_NAME_RAW <- Sys.getenv("ABLATION_NAME", unset = "")
ABLATION_NAME <- gsub("[^a-zA-Z0-9_-]", "_", ABLATION_NAME_RAW)
ABLATION_NAME <- gsub("_+", "_", ABLATION_NAME)
ABLATION_NAME <- gsub("^_|_$", "", ABLATION_NAME)

# Reference policy override (for consistent ablation comparisons)
# LEGACY DEFAULT: alerts_per_day_cap = 10 (preserves backward compatibility for standalone runs)
# For orchestrated ablations, pass env vars to set headline policy (e.g., precision_floor = 0.10)
VALID_POLICY_TYPES <- c("precision_floor", "alerts_per_day_cap", "f1_max")
REFERENCE_POLICY_TYPE <- Sys.getenv("REFERENCE_POLICY_TYPE", unset = "alerts_per_day_cap")
if (!REFERENCE_POLICY_TYPE %in% VALID_POLICY_TYPES) {
  stop("Invalid REFERENCE_POLICY_TYPE: '", REFERENCE_POLICY_TYPE,
       "'. Must be one of: ", paste(VALID_POLICY_TYPES, collapse = ", "))
}

# Reference policy target (policy-type-specific validation and defaults)
REFERENCE_POLICY_TARGET_RAW <- Sys.getenv("REFERENCE_POLICY_TARGET", unset = "")
if (REFERENCE_POLICY_TYPE == "f1_max") {
  # f1_max has no target (NA)
  REFERENCE_POLICY_TARGET <- NA_real_
  if (REFERENCE_POLICY_TARGET_RAW != "" && REFERENCE_POLICY_TARGET_RAW != "NA") {
    message("  Note: REFERENCE_POLICY_TARGET ignored for f1_max policy (using NA)")
  }
} else if (REFERENCE_POLICY_TYPE == "precision_floor") {
  # precision_floor: must be in (0, 1], default 0.10
  if (REFERENCE_POLICY_TARGET_RAW == "") {
    REFERENCE_POLICY_TARGET <- 0.10
  } else {
    REFERENCE_POLICY_TARGET <- suppressWarnings(as.numeric(REFERENCE_POLICY_TARGET_RAW))
    if (is.na(REFERENCE_POLICY_TARGET) || REFERENCE_POLICY_TARGET <= 0 || REFERENCE_POLICY_TARGET > 1) {
      stop("Invalid REFERENCE_POLICY_TARGET for precision_floor: '", REFERENCE_POLICY_TARGET_RAW,
           "'. Must be numeric in (0, 1]")
    }
  }
} else if (REFERENCE_POLICY_TYPE == "alerts_per_day_cap") {
  # alerts_per_day_cap: must be > 0, default 10
  if (REFERENCE_POLICY_TARGET_RAW == "") {
    REFERENCE_POLICY_TARGET <- 10
  } else {
    REFERENCE_POLICY_TARGET <- suppressWarnings(as.numeric(REFERENCE_POLICY_TARGET_RAW))
    if (is.na(REFERENCE_POLICY_TARGET) || REFERENCE_POLICY_TARGET <= 0) {
      stop("Invalid REFERENCE_POLICY_TARGET for alerts_per_day_cap: '", REFERENCE_POLICY_TARGET_RAW,
           "'. Must be numeric > 0")
    }
  }
}

# Feature filtering for ablations (regex patterns)
# If set, only features matching INCLUDE and not matching EXCLUDE will be used
# Empty string means no filter (use all features)
FEATURE_INCLUDE_REGEX <- Sys.getenv("FEATURE_INCLUDE_REGEX", unset = "")
FEATURE_EXCLUDE_REGEX <- Sys.getenv("FEATURE_EXCLUDE_REGEX", unset = "")

# Output root override (for ablation batch organization)
# If set, OUTPUT_DIR will be placed under this root instead of default location
OUTPUT_ROOT_OVERRIDE <- Sys.getenv("OUTPUT_ROOT_OVERRIDE", unset = "")

# Features path override (for using pre-filtered or alternative feature files)
# If set, use this path instead of default model_ready/features_engineered.rds
FEATURES_PATH_OVERRIDE <- Sys.getenv("FEATURES_PATH_OVERRIDE", unset = "")

# Test date exclusion (for sensitivity analysis, e.g., Texas shock 2021-02-15:2021-02-17)
# Format: comma-separated ISO dates, e.g., "2021-02-15,2021-02-16,2021-02-17"
TEST_EXCLUDE_DATES_RAW <- Sys.getenv("TEST_EXCLUDE_DATES", unset = "")
TEST_EXCLUDE_DATES <- if (TEST_EXCLUDE_DATES_RAW != "") {
  as.Date(trimws(strsplit(TEST_EXCLUDE_DATES_RAW, ",")[[1]]))
} else {
  as.Date(character(0))
}

# Spatial holdout (for robustness analysis, e.g., hold out South region)
# Format: comma-separated state names, e.g., "Guerrero,Oaxaca,Chiapas"
# When set: train+val EXCLUDE these states, test ONLY includes these states
# Names are normalized (uppercase, ASCII) to match state_clean column
SPATIAL_HOLDOUT_STATES_RAW <- Sys.getenv("SPATIAL_HOLDOUT_STATES", unset = "")
SPATIAL_HOLDOUT_STATES <- if (SPATIAL_HOLDOUT_STATES_RAW != "") {
  stringi::stri_trans_general(
    toupper(trimws(strsplit(SPATIAL_HOLDOUT_STATES_RAW, ",")[[1]])),
    "Latin-ASCII"
  )
} else {
  character(0)
...
# BUILD RUN_ID (after all hooks are parsed)
mode_suffix <- if (STRICT_CALIBRATION_THRESHOLD_SEPARATION) "strict" else "standard"
ablation_suffix <- if (ABLATION_NAME != "") paste0("_", ABLATION_NAME) else ""
RUN_ID <- paste0(format(Sys.time(), "%Y%m%d_%H%M%S"), "_", task_mode, "_", mode_suffix, ablation_suffix)
...
if (ABLATION_NAME != "") {
  message("  Ablation name: ", ABLATION_NAME, if (ABLATION_NAME != ABLATION_NAME_RAW) paste0(" (raw: '", ABLATION_NAME_RAW, "')") else "")
}
message("  Reference policy: ", REFERENCE_POLICY_TYPE, " = ",
        if (is.na(REFERENCE_POLICY_TARGET)) "NA" else REFERENCE_POLICY_TARGET)
if (FEATURE_INCLUDE_REGEX != "") message("  Feature include regex: ", FEATURE_INCLUDE_REGEX)
if (FEATURE_EXCLUDE_REGEX != "") message("  Feature exclude regex: ", FEATURE_EXCLUDE_REGEX)
if (OUTPUT_ROOT_OVERRIDE != "") message("  Output root override: ", OUTPUT_ROOT_OVERRIDE)
if (FEATURES_PATH_OVERRIDE != "") message("  Features path override: ", FEATURES_PATH_OVERRIDE)
if (length(TEST_EXCLUDE_DATES) > 0) message("  Test exclude dates: ", paste(TEST_EXCLUDE_DATES, collapse = ", "))
if (length(SPATIAL_HOLDOUT_STATES) > 0) message("  Spatial holdout states: ", paste(SPATIAL_HOLDOUT_STATES, collapse = ", "))
...
# If OUTPUT_ROOT_OVERRIDE is set, use it as the root for ablation batch organisation
if (OUTPUT_ROOT_OVERRIDE != "") {
  OUTPUT_DIR <- file.path(OUTPUT_ROOT_OVERRIDE, RUN_ID)
} else {
...
# Use FEATURES_PATH_OVERRIDE if set, otherwise default to model_ready location
if (FEATURES_PATH_OVERRIDE != "") {
  features_path <- FEATURES_PATH_OVERRIDE
  message("Using features path override: ", features_path)
} else {
...
# Ablation runs always use weather_mode="lagged" in forecast mode (truly forecast-safe).
...
# Same-day weather is dropped via build_exclude_cols(task_mode, weather_mode="lagged") above,
# not here. apply_task_mode_filters handles NTL/anomaly/nb_* drops.
...
# Apply test date exclusion if specified (for sensitivity analysis)
if (length(TEST_EXCLUDE_DATES) > 0) {
  test_before <- nrow(test_data)
  pos_before <- sum(test_data$outage_3h_or_more)
  test_data <- test_data %>% filter(!date %in% TEST_EXCLUDE_DATES)
  test_after <- nrow(test_data)
  pos_after <- sum(test_data$outage_3h_or_more)
  message("\nTest date exclusion applied:")
  message("  Excluded dates: ", paste(TEST_EXCLUDE_DATES, collapse = ", "))
  message("  Rows removed: ", test_before - test_after, " (",
          round(100 * (test_before - test_after) / test_before, 2), "%)")
  message("  Positives removed: ", pos_before - pos_after)
}

# Apply spatial holdout if specified (for robustness analysis)
if (length(SPATIAL_HOLDOUT_STATES) > 0) {
  # Determine filter column (prefer state_clean if available)
  state_filter_col <- if ("state_clean" %in% names(features)) "state_clean" else "NAME_1"

  # Normalize column values for matching if using state_clean
  if (state_filter_col == "state_clean") {
    train_states <- train_data[[state_filter_col]]
    val_states <- val_data[[state_filter_col]]
    test_states <- test_data[[state_filter_col]]
  } else {
    # Normalize NAME_1 for matching
    train_states <- stringi::stri_trans_general(toupper(train_data[[state_filter_col]]), "Latin-ASCII")
    val_states <- stringi::stri_trans_general(toupper(val_data[[state_filter_col]]), "Latin-ASCII")
    test_states <- stringi::stri_trans_general(toupper(test_data[[state_filter_col]]), "Latin-ASCII")
  }

  train_before <- nrow(train_data); train_pos_before <- sum(train_data$outage_3h_or_more)
  val_before <- nrow(val_data); val_pos_before <- sum(val_data$outage_3h_or_more)
  test_before <- nrow(test_data); test_pos_before <- sum(test_data$outage_3h_or_more)

  # Filter: train+val EXCLUDE holdout, test ONLY holdout
  train_data <- train_data[!train_states %in% SPATIAL_HOLDOUT_STATES, ]
  val_data <- val_data[!val_states %in% SPATIAL_HOLDOUT_STATES, ]
  test_data <- test_data[test_states %in% SPATIAL_HOLDOUT_STATES, ]

  message("\nSpatial holdout applied:")
  message("  Filter column: ", state_filter_col)
  message("  Holdout states: ", paste(SPATIAL_HOLDOUT_STATES, collapse = ", "))
  message("  Train: ", format(train_before, big.mark = ","), " -> ", format(nrow(train_data), big.mark = ","),
          " rows (removed ", train_before - nrow(train_data), ", ",
          train_pos_before - sum(train_data$outage_3h_or_more), " positives)")
  message("  Val:   ", format(val_before, big.mark = ","), " -> ", format(nrow(val_data), big.mark = ","),
          " rows (removed ", val_before - nrow(val_data), ", ",
          val_pos_before - sum(val_data$outage_3h_or_more), " positives)")
  message("  Test:  ", format(test_before, big.mark = ","), " -> ", format(nrow(test_data), big.mark = ","),
          " rows (kept ", nrow(test_data), ", ", sum(test_data$outage_3h_or_more), " positives)")
  message("  Train prevalence after holdout: ", round(mean(train_data$outage_3h_or_more), 5))
  message("  Val prevalence after holdout: ", round(mean(val_data$outage_3h_or_more), 5))
  message("  Test prevalence after holdout: ", round(mean(test_data$outage_3h_or_more), 5))
}
...
# Apply FEATURE_INCLUDE_REGEX and FEATURE_EXCLUDE_REGEX if set
# These allow orchestrated ablation runs to select feature subsets
if (FEATURE_INCLUDE_REGEX != "" || FEATURE_EXCLUDE_REGEX != "") {
  n_before_filter <- length(feature_cols)

  # Include filter: keep only features matching the pattern
  if (FEATURE_INCLUDE_REGEX != "") {
    include_matches <- grepl(FEATURE_INCLUDE_REGEX, feature_cols, perl = TRUE)
    if (!any(include_matches)) {
      stop("FEATURE_INCLUDE_REGEX '", FEATURE_INCLUDE_REGEX,
           "' matched 0 features. Check pattern or available features.")
    }
    feature_cols <- feature_cols[include_matches]
    message("  INCLUDE filter matched ", sum(include_matches), " features")
  }

  # Exclude filter: remove features matching the pattern
  if (FEATURE_EXCLUDE_REGEX != "") {
    exclude_matches <- grepl(FEATURE_EXCLUDE_REGEX, feature_cols, perl = TRUE)
    n_excluded <- sum(exclude_matches)
    feature_cols <- feature_cols[!exclude_matches]
    message("  EXCLUDE filter removed ", n_excluded, " features")
  }

  # Guard: must have at least 1 feature remaining
  if (length(feature_cols) == 0) {
    stop("Feature filtering resulted in 0 features. ",
         "Include: '", FEATURE_INCLUDE_REGEX, "', Exclude: '", FEATURE_EXCLUDE_REGEX, "'")
  }

  message("  After ablation filtering: ", n_before_filter, " -> ", length(feature_cols), " features")
}
...
# This table summarizes practical operating points for a NO-FIXED-BUDGET setting
# Thresholds are selected on VALIDATION, then evaluated on TEST
# Includes multiple policy types:
#   - precision_floor: maximize recall subject to precision >= target
...
# Ensure reference policy is present in policy_configs
# This allows orchestrated ablations to use custom policies not in the default grid
policy_exists <- function(policy_list, ref_type, ref_target) {
  any(vapply(policy_list, function(p) {
    same_type <- identical(p$type, ref_type)
    same_target <- (is.na(p$target) && is.na(ref_target)) ||
      (!is.na(p$target) && !is.na(ref_target) && isTRUE(all.equal(p$target, ref_target)))
    same_type && same_target
  }, logical(1)))
}

if (!policy_exists(policy_configs, REFERENCE_POLICY_TYPE, REFERENCE_POLICY_TARGET)) {
  policy_configs <- append(policy_configs, list(list(
    type = REFERENCE_POLICY_TYPE,
    target = REFERENCE_POLICY_TARGET
  )))
  message("  Added reference policy to policy grid: ",
          REFERENCE_POLICY_TYPE, " = ",
          ifelse(is.na(REFERENCE_POLICY_TARGET), "NA", as.character(REFERENCE_POLICY_TARGET)))
}
...
# Use env-driven values from ablation hooks (allows orchestrated runs to control headline policy)
reference_policy_type <- REFERENCE_POLICY_TYPE
reference_policy_target <- REFERENCE_POLICY_TARGET

# NA-safe policy target matching helper
# Required because NA == NA returns NA, not TRUE (breaks f1_max policy filtering)
match_policy_target <- function(col_target, ref_target) {
  if (is.na(ref_target)) {
    return(is.na(col_target))
  } else {
    return(!is.na(col_target) & dplyr::near(col_target, ref_target))
  }
}

# Pretty-format policy target for messages (handles NA and decimals)
format_policy_target_label <- function(x) {
...
for (method in benchmark_methods) {
  ...
  # Get operating point metrics (NA-safe filter for f1_max)
  op_row <- operating_thresholds_table %>%
    filter(method == !!method,
           policy_type == reference_policy_type) %>%
    filter(match_policy_target(policy_target, reference_policy_target))
...
for (method in benchmark_methods) {
  ...
  if (nrow(bm_row) > 0) {
    message(sprintf("    %s: logloss=%.4f, ece=%.4f, recall@%s=%s: %.3f",
                    method,
                    bm_row$logloss_test,
                    bm_row$ece_equal_test,
                    reference_policy_type,
                    format_policy_target_label(reference_policy_target),
...
reference_policy_row <- operating_thresholds_table %>%
  filter(policy_type == reference_policy_type) %>%
  filter(match_policy_target(policy_target, reference_policy_target))
...
run_config <- list(
  ...
  # Ablation hooks (for orchestrated runs; empty/default for standalone)
  ablation = list(
    ablation_name = if (ABLATION_NAME != "") ABLATION_NAME else NULL,
    ablation_name_raw = if (ABLATION_NAME_RAW != "") ABLATION_NAME_RAW else NULL,
    reference_policy_type = REFERENCE_POLICY_TYPE,
    reference_policy_target = REFERENCE_POLICY_TARGET,
    feature_include_regex = if (FEATURE_INCLUDE_REGEX != "") FEATURE_INCLUDE_REGEX else NULL,
    feature_exclude_regex = if (FEATURE_EXCLUDE_REGEX != "") FEATURE_EXCLUDE_REGEX else NULL,
    output_root_override = if (OUTPUT_ROOT_OVERRIDE != "") OUTPUT_ROOT_OVERRIDE else NULL,
    features_path_override = if (FEATURES_PATH_OVERRIDE != "") FEATURES_PATH_OVERRIDE else NULL,
    test_exclude_dates = if (length(TEST_EXCLUDE_DATES) > 0) as.character(TEST_EXCLUDE_DATES) else NULL,
    spatial_holdout_states = if (length(SPATIAL_HOLDOUT_STATES) > 0) SPATIAL_HOLDOUT_STATES else NULL
  ),
